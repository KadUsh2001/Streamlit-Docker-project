...
# initial data engineering on the dataset
...
# sidebar which allows to select type of analysis
type_of_an = st.radio(
    "What type of analysis would you like?",
    ("Season only", "Team only", "Season and team"),
)
...
if type_of_an == "Season only":
    ...
    if st.checkbox("Start analysis"):
        ...
        with col1:
            # League table dataframe visualization
            st.header("League table")
            st.dataframe(league_table, hide_index=True)

        with col2:
            teams_selected = st.multiselect(
                "Which teams's points do you want to see",
                np.sort(
                    np.array(season_df["Team"]),
                ),
            )
            if len(teams_selected) > 0:
                # bar plot of how many points per team during the season for the teams selected
...
if type_of_an == "Team only":
    # selectbox to select team for analysis
    team = st.selectbox(
        "Which team?",
        (
            "Arsenal",
    ...
    if st.checkbox("Start analysis"):
        ...
        seasons_selected = st.multiselect(
            "Which seasons do you want to see",
            np.sort(
                np.array(team_df["Season"]),
            ),
        )
        col6, col7, col8 = st.columns(3)
        with col6:
            if len(seasons_selected) > 0:
                # Allows to check for team position for the seasons selected
                seasons_selected_data = team_df[
                    team_df["Season"].isin(seasons_selected)
                ]
        ...
        with col7:
            if len(seasons_selected) > 0:
                # gives some general information about the results obtained by the team in the seasons selected
                seasons_selected_data = team_df[
                    team_df["Season"].isin(seasons_selected)
                ]
        ...
        with col8:
            if len(seasons_selected) > 0:
                # bar plot with number of wins draws and losses per team
                seasons_selected_data = team_df[
                    team_df["Season"].isin(seasons_selected)
                ]
...
if type_of_an == "Season and team":
    # slider and selectbox to select season and team
    season = st.slider("Which season?", min_value=2001, max_value=2022, step=1)
    season_string = str(season)
    season_df = data[data["Season"] == season_string]
    ...
    if st.checkbox("Start analysis"):
        ...
        with col9:
            ...
            st.pyplot(fig)
        with col10:
            # general information regarding the results obtained by the team during the season
            season_team_df = season_df[season_df["Team"] == team_string]
        ...
        with col11:
            # bar plot of amounts of wins draws and losses
